...
def getFeatureVector(nodeList):
    ...
    8) Get number of full stops in direct text
    '''
    featureNodes = []
...
def getTree(s):
    ...
    '''
    l = len(s)
    st, curString, cst, headers, isFirst = [], [], [], [], False
    i = -1
    while (i<l-1):
        i += 1
        if (s[i] == '<'):
            if (s[i+1] != '/'):
                ...
                spanTag = False
                if( (about.find('''class="x-nc-sel2"''') != -1) or (about.find('''class="x-nc-sel1"''') != -1)):
                    spanTag = True
                    cst.append(n)
